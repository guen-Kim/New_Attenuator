...
namespace New_Attenuator
{
    public partial class Form1 : Form
    {
        ...
        private void SetupModeToolTips()
        {
            ...
            modeToolTip.SetToolTip(rbBasic1, "Cross Fade Test: 2개 AP를 서로 반대 방향으로 감쇠시키는 교차 제어 모드입니다.");
            modeToolTip.SetToolTip(rbBasic2, "Sequential Sweep Test: 선택된 AP를 순서대로 하나씩 start에서 end까지 스윕합니다.");
            modeToolTip.SetToolTip(rbBasic3, "Group Sweep Test: 모든 활성 AP를 같은 값으로 동시에 올리고 내리는 기준 동작입니다.");
            modeToolTip.SetToolTip(rbTrans1, "Dual Pair Handover Test: AP1/AP3와 AP2/AP4를 순차적으로 약화시키는 전환 테스트입니다.");
            modeToolTip.SetToolTip(rbTrans2, "Parallel Pair Handover Test: 2개 AP 쌍을 동시에 교차 전환하는 병렬 handover 테스트입니다.");
            modeToolTip.SetToolTip(rbTrans3, "Sequential Roaming Test: 4개 AP를 순서대로 하나씩 전환하는 로밍 테스트입니다.");
            modeToolTip.SetToolTip(rbTrans4, "Smooth Handover Test: AP가 겹치며 다음 AP로 자연스럽게 넘어가는 handover 테스트입니다.");
            modeToolTip.SetToolTip(rbStepHandover, "Step Handover Test: AP를 한 칸씩 순차적으로 넘기는 기본 handover 테스트입니다.");
            modeToolTip.SetToolTip(rbPingPong, "Ping-Pong Handover Test: 두 AP 사이를 반복 전환하는 안정성 테스트입니다.");
            modeToolTip.SetToolTip(rbDiagonal, "Diagonal Handover Test: 비연속 AP로 건너뛰는 전환 경로 테스트입니다.");
            modeToolTip.SetToolTip(rbFailover, "Failover Recovery Test: 주 AP 장애 시 백업 AP로 복구하는 장애 대응 테스트입니다.");
        }
